...
class BSIBankStatement(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('transactions', 'Transactions'),('data_updated', 'Data Updated'), ('error', 'Error')], tracking=True)
                ...
                for txn in account_transactions:
                    ...
                    if key in existing_txn_keys:
                        ...
                        new_txn = TransactionModel.create({
                            'account_id':       acc_rec.id,
                            'store_id':         acc_rec.store_id.id if acc_rec.store_id else False,
    ...
    def update_both_foodcost_payroll(self):
        ...
        for rec in self:
            ...
            # ── STEP 1: account must have a store ───────────────────────────────
            ...
            # ── STEP 2: account must have transactions ──────────────────────────
            ...
            # ── STEP 3: store must have scheduling records ──────────────────────
            ...
            # ── STEP 4: choose mappings ─────────────────────────────────────────
            #   CASE 1: account number IS in mapping  → use those (prev_week default 1)
            #   CASE 2: account number NOT in mapping → use generic mappings (prev_week default 0)
            ...
            # ── STEP 5: process each mapping (same flow for both cases) ─────────
            for mapping in mappings:
                keyword       = str(mapping.keyword or '').strip().lower()
                ...
                # validation: keyword / field name missing → popup
                ...
                # validation: field name must be a real field on store.scheduling → popup
                ...
                # match keyword in THIS account's transaction descriptions
                matched = [
                    t for t in account_transactions
                    if keyword in str(t.description or '').strip().lower()
                ]
                ...
                for t in matched:
                    ...
                    # find the scheduling week whose start/end date contains this txn date
                ...
                # write the amounts into the field on each target scheduling week
            ...
            # ── STEP 6: state change based on outcome ───────────────────────────
            if update_log:
                rec.write({'state': 'data_updated', 'error': False})
                rec.message_post(body="Update both completed.<br/>" + "<br/>".join(update_log))
