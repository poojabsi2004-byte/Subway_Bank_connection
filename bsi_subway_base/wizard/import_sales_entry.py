...
class ImportSalesEntry(models.TransientModel):
    _name = "bsi.import.sales.entry.wizard"
    _description = "Import Sales Entry"
    ...
    def import_button(self):
        ...
        for row_no in range(sheet.nrows):
            ...
            if row_no > 0:
                ...
                for rec in line:
                    if col_count >=0 and col_count <=2:
                        if col_count == 0:
                            store_id = self.env['store.store'].search([('name', '=', line[col_count])])
                            if not store_id:
                                store_id = self.env['store.store'].create({'name': line[col_count], 'store_number': float(line[col_count+1])})
                    if col_count >=3:
                        ...
                        'net_sales':line[col_count]
                        }
                        bsi_store_id = self.env['store.scheduling'].create(vals)
                        start_date = start_date + timedelta(days=7)
                    col_count += 1

    def import_payrollsheet_button(self):
        data_dict ={}
        try:
            xlsfile = tempfile.NamedTemporaryFile(delete= False,suffix=".xlsx")
            xlsfile.write(binascii.a2b_base64(self.file))
        ...
        for row_no in range(sheet.nrows):
            ...
            # RM - 14 Aug [ADD] update_store_scheduling button code merge to import_payrollsheet_button   
            if row_no == 1:
                line = list(map(lambda row:isinstance(row.value, bytes) and row.value.encode('utf-8') or str(row.value), sheet.row(row_no)))
                col_count = 0
                sub_line_count = 0
